...
```
用户: 上传 lynxi_driver_hm100 到板卡
AI:
  1. 解析 .reproject 文件
  2. 验证网络连通性（ping 板卡 IP）
  3. 通过 FTP 上传文件
  4. 报告上传结果
```
...
- 使用 `multi-platform.mk` 作为构建入口
- 所有路径必须使用绝对路径
- 支持 ARM64_GENERIC 等平台
- 自动处理 libdrv_linux_compat 依赖
...
| 文件类型 | 扩展名 | 目标路径 |
|---------|--------|---------|
| 驱动模块 | *.ko | /lib/modules/drivers/ |
| 共享库 | *.so | /lib/ |
| 静态库 | *.a | /lib/ |
| 可执行文件 | 无扩展名 | /apps/ 或 /usr/bin/ |
| 测试工具 | 无扩展名 | /apps/xxx_test/ |
